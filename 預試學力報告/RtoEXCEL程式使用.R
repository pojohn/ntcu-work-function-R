...
source("C:\\Users\\user\\Documents\\台中教育大學\\學力工作\\00程式需求\\預試程式\\RtoEXCEL程式(第一版0616).R", encoding = "utf-8")
source("C:\\Users\\user\\Documents\\台中教育大學\\學力工作\\00程式需求\\預試程式\\RtoEXCEL程式(第二版0616).R", encoding = "utf-8")
...
####################################
# data.path = choose.files()
 data.path = "C:\\Users\\user\\Documents\\台中教育大學\\學力工作\\20200526-學力預試報告(畫圖)\\109年學力預試報告\\數學\\苗栗-六合國小六年級數學報告109年學力預試2.xlsx"
 
 data   <-read_excel(data.path,sheet = 1) %>% data.table()
 soc.all<-read_excel(data.path,sheet = 2) %>% data.table()
#####################################
school.score.plot.cc(data.path,data,soc.all.in.one,key,plot.turn = T)
  
school.score.plot.cc.2(data.path,data,soc.all ,plot.turn = T)

# 備份 6/16
school.score.plot.cc<-function(data.path,data,soc.all.in.one,key) {
  # 建立 workbook
  wb <- createWorkbook()
  ## 取得 檔名
  ...
  ## 儲存新檔案
  saveWorkbook(wb, paste( "C:/Users/user/Desktop/",school,".xlsx",  sep=""), overwrite = TRUE)
